...
public class CardPlugin extends BotPlugin {
    ...
    public int onGroupMessage(@NotNull Bot bot, @NotNull OnebotEvent.GroupMessageEvent event) {
        ...
        if (qqGroupSerivce.selectAllByID(String.valueOf(groupId)).getDiceOpen() == 0) {
            ...
            if (msg.startsWith(".ra")) {
                return appraisalSkill(bot, msg, groupId, userId, nickname);
            }
     ...
     * @date 2021/4/5
     */
    private int appraisalSkill(@NotNull Bot bot, String msg, long groupId, long userId, String nickname) {
        try {
            //获取默认人物卡
            ...
            Random random = new Random();
            //投掷骰子
            int skillResult = random.nextInt(101);
            //判断是否有加值
            if (skill.indexOf("+") != -1) {
                String[] skills = skill.split("\\+");
                skill = skills[0];
                skillValue = Integer.valueOf(skills[1]);
                symbol = "+";
            }
            if (skill.indexOf("-") != -1) {
                String[] skills = skill.split("\\-");
            ...
            //查询对应技能
            SkillCoC skillCoC = skillCocService.getOne(Wrappers.<SkillCoC>lambdaQuery().eq(SkillCoC::getCharacterId, characterCoC.getId()).eq(SkillCoC::getName, skill));
            if (skillCoC == null) {
                bot.sendGroupMsg(groupId, "没有该技能呢...重新设置下试试吧!", false);
                return MESSAGE_IGNORE;
            }
            //判断昵称是否为空
            if (StringUtils.isNotBlank(characterCoC.getName())) {
            ...
            if (StringUtils.isNotBlank(symbol)) {
                ...
                builder.append(symbol + skillValue + "检定：1D100=" + skillResult + "/" + (skillValue + skillCoC.getValue()));
            } else {
                builder.append("检定：1D100=" + skillResult + "/" + (skillCoC.getValue()));
            }
            //判断结果
            if (skillResult <= 5) {
                builder.append(",大成功！看到了吗？这满天的星光，都只为你而闪耀。");
            } else if (skillResult >= 100) {
                builder.append(",大失败...当浩瀚星海都已燃尽黯淡的现在，这个世界…已经没有未来了吧......");
            } else if (skillResult < ((skillCoC.getValue() + skillValue) / 5)) {
                builder.append(",极限成功！真是稀奇呢。不，原来是你的努力点亮了那片璀璨的星光吗？");
            } else if (skillResult < ((skillCoC.getValue() + skillValue) / 2)) {
                builder.append(",困难成功！愿闪耀的启明星为你指引命运的方向。");
            } else if (skillResult < (skillCoC.getValue() + skillValue)) {
